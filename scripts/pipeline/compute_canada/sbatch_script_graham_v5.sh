...
do
   parallel --env _ -j 32 --will-cite copy_structure $(basename ${prnt}) {} ${SLURM_TMPDIR} ::: $(find ${BASE}/${prnt} -type f)
done

rsync -Lar ${HOME}/.etetoolkit/taxa.sqlite ${R1} ${R2} ${BASE}/split_files/file_chunk_1 ${SLURM_TMPDIR}
