...
# Create function to create taxlist vector with phylum per row in taxonomy table (or if phylum = NA, then superkingdom)
make_tax_group_dataframe <- function(tax)
  {
    taxlastrow <- nrow(tax)
...
# Apply fuction to list of dataframes
tax_group_dataframes <- lapply(tax_full_dataframes, make_tax_group_dataframe) 
...
# Determine columns for different combinations
