...
# 1 Import clustering info (doesn;t matter if from gen or cell reference abundances, they're the same)
pd.read_csv(os.path.join(workdir, "results_{0}_cell".format(groupby_rank), "stats_exports", "rel_cluster_info.csv"))
...
for master_dfs in [master_dfs_rel_sub, master_dfs_pa]:
    ...
    ## Save the df
    std_df.to_csv(os.path.join(exportdir, "master_df_env_" + data_type + ".csv"), index_label="pipeline")
