...
if looping:
    statsdir_lst=[os.path.join(workdir, "metrics_{0}".format(x)) \
        for x in ["cell_genus", "gen_genus", "cell_species", "gen_species"]]
    metr_list=["all", "rel", "pa"]
else:
    statsdir_lst=[os.path.join(workdir, "metrics_" + combination)]
    metr_list=[metrics]
...
for statsdir in statsdir_lst:
    for metr in metr_list:
        ...
        #
        # pval={}
        # for col in X_no_intercept.columns:
        #     pval[col]=pearsonr(X_no_intercept[col], y)
        #
        # from scipy.stats import pearsonr
        #
        # def calculate_pvalues(df):
        #     df = X_no_intercept.dropna()._get_numeric_data()
        ...
        #     return pvalues
        #
        # pval_df=copy.deepcopy(X_no_intercept)
        # pval_df["euc"]=y
        # calculate_pvalues(pval_df)["euc"]
        ...
        #kmean_df_std=euc_dist_df["euc_dist"].array.reshape(-1,1) # For cluster based on euc_dist
        ...
        # 7.4 Generate average euc distance for each cluster
        cluster_df=pd.DataFrame({"pipeline": pca_df.index.to_list(), "cluster":[str(x) for x in kmeans.labels_.tolist()], "euc_dist":euc_dist_df["euc_dist"]})
        cluster_spread_mean_euc={}
        for cluster in set(cluster_df["cluster"]):
            cluster_spread_mean_euc[cluster]=cluster_df.loc[cluster_df['cluster'] == cluster]["euc_dist"].mean()


        # 7.4 Export k and the clustering information to compare it to env samples
        ...
        with open(os.path.join(statsdir, metr + "_k.pkl"), 'wb') as f:
            pickle.dump(k, f)
