...
${R1} ${R2} ${BASE}/split_files_silva_32_pips/file_chunk13.txt \
...
run_it(){
  ...
  mkdir -p ${pipeline}
  mkdir -p ${SLURM_TMPDIR}/${pipeline}
  cp ${pipeline}/*.fa* ${pipeline}/merge_input_mapped_*.txt ${SLURM_TMPDIR}/${pipeline}
  cd ${SLURM_TMPDIR}/${pipeline}
