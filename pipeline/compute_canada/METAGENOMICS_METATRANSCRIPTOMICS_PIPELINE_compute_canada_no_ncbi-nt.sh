...
## This script excludes ncbi-nt due to its high RAM requirements for BLAST and especially kraken2
## Therefore, if ncbi-nt is part of the pipeline, the script will stop here, copy the resulting scaffolds and mapping data to the final files folder, and exit

cp $scaffolds step_4_mapping/$(echo $mapping | tr '[:lower:]' '[:upper:]')/merge_input_mapped_${mapping}.txt \
${base_directory}/METAGENOMICS_METATRANSCRIPTOMICS_PIPELINE_FINAL_FILES/

if [[ $db == 'ncbi_nt' ]]; then
  echo "Pipeline contains ncbi-nt. Scaffolds and mapper output was copied and the script will exit now."
  exit
fi
